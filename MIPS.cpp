...
string operation;
char word[5][4];
bool IF_start = true, ID_start = true, EX_start = true, MEM_start = true, WB_start = true, isbeq = false;
...
void IF() {
    ...
    if (last_line == true)current_line++;
    input.close();
...
void ID() {
    ...
    if (isbeq) {
        ...
        if (blank == 2) {
            if (operation[0] == 'l') {  //lw
                EX_RegDst = 0, EX_ALUSrc = 1, EX_Branch = 0, EX_MemRead = 1, EX_MemWrite = 0, EX_RegWrite = 1, EX_MemtoReg = 1;
                function = LW;
                ...
                EX_RegDst = 2, EX_ALUSrc = 1, EX_Branch = 0, EX_MemRead = 0, EX_MemWrite = 1, EX_RegWrite = 0, EX_MemtoReg = 2;
                function = SW;
            ...
            if (space[1] + 1 == '-') {  //negative
                if (convert_int(space[1], 3, CHECK)) {
                    number = -(convert_int(space[1], 2, DOUBLE_DIGITS)) / 4;
                    ...
                    number = -(convert_int(space[1], 2, SINGLE)) / 4;
            ...
            if (operation[0] == 'b') { //beq
                EX_RegDst = 2, EX_ALUSrc = 0, EX_Branch = 1, EX_MemRead = 0, EX_MemWrite = 0, EX_RegWrite = 0, EX_MemtoReg = 2;
                function = BEQ;
            }
            else if (operation[0] == 'a') {  //add
                EX_RegDst = 1, EX_ALUSrc = 0, EX_Branch = 0, EX_MemRead = 0, EX_MemWrite = 0, EX_RegWrite = 1, EX_MemtoReg = 0;
                function = ADD;
            }
            else if (operation[0] == 's') { //sub
                EX_RegDst = 1, EX_ALUSrc = 0, EX_Branch = 0, EX_MemRead = 0; EX_MemWrite = 0, EX_RegWrite = 1, EX_MemtoReg = 0;
                function = SUB;
            }
...
int main() {
    ...
    while (true)
    {
        ...
        if (!IF_start && !ID_start && !EX_start && !MEM_start && !WB_start) break;
        cycle_count++;
        stall_count--;
    }

    output.open("result.txt", ios::out | ios::app);
    output << endl << "Need " << cycle_count << " cycles" << endl;
    for (int i = 0; i < 32; i++)
        output << "$" << i << "\t";
    output << endl;
    for (int i = 0; i < 32; i++)
